 ...
 * @author Sushant
 */
public class ManageParkRequestsPanel extends javax.swing.JPanel {
    EcoSystem system;
    /**
     * Creates new form ManageRequestsPanel
     */
    public ManageParkRequestsPanel(EcoSystem system) {
        initComponents();
    ...
    public void populateParkRequestsTable() {
        ...
        ArrayList<ParkingWorkRequest> non_pendingParkingRequest = new ArrayList<>();
        ...
        for (ParkingWorkRequest parkingWRequest : system.getParkingWorkRequestDirectory().getParkingWorkRequestList()) {
            if (parkingWRequest.getStatus().equals("Pending")) {
                ...
                non_pendingParkingRequest.add(parkingWRequest);
            }
        }

        populate_based_on_status(pendingParkingRequest, non_pendingParkingRequest);
    }

    public void populate_based_on_status(ArrayList<ParkingWorkRequest> pendingParkingRequest, ArrayList<ParkingWorkRequest> non_pendingParkingRequest) {
        ...
        DefaultTableModel modelNon_Pending = (DefaultTableModel) tblParkConfirmed.getModel();
        modelNon_Pending.setRowCount(0);
        for (ParkingWorkRequest parkingWRequest : non_pendingParkingRequest) {
    ...
    public void updateWorkRequestStatus(ParkingWorkRequest selectedParking, String status) {
        ...
        int index = 0;
        for (ParkingWorkRequest parkingWRequest : system.getParkingWorkRequestDirectory().getParkingWorkRequestList()) {
            if (parkingWRequest.getUserId().equals(selectedParking.getUserId())) {
    ...
    private void btnApproveActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnApproveActionPerformed
        ...
        try {
            ...
            User getUser = system.getUserDirectory().getUserById(selectedParkingWorkRequest.getUserId());

            float parking_finalPrice = selectedParkingWorkRequest.getNumberOfHours() * selectedParkingWorkRequest.getParkingDetails().getPrice();
            ct.setUserId(selectedParkingWorkRequest.getUserId());
            ...
            ct.setPrice(parking_finalPrice);
            system.getCustomerTransactionDirectory().addCustomerTransaction(ct);
            
            new EmailNotification().SendEmailOfNotification(getUser, "Parking");
